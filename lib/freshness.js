...
export function freshness(map, ctx = {}) {
  ...
  // キャッシュに入った時刻 ＝ 受信時刻 − Age。Fastly 等は Date ヘッダーを「配信時刻」に
  // 書き換えるため、Date をそのまま作成時刻に使うと age ぶん新しくズレる。Age を引く
  // （RFC 9111 の current_age = Age から遡る考え方）。Age 不在時は受信時刻で代替。
  const createdAt = new Date(receiveTime - (ageHeader ?? 0) * 1000);
