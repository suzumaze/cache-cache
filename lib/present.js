...
function originView(v) {
  ...
  if (st === 'server') {
    ...
    return { icon: 'host', label: 'サーバー', lead: 'サーバーから届きました（その場で作ったか、用意していた控えかまでは分かりませんでした）', badge: { text: '源', color: COLOR.server, textColor: '#FFFFFF' } };
  }
...
export function present(v) {
  ...
  if (v.cdn?.name === 'Fastly') {
    rows.push(v.cdn.pops >= 2
      ? { label: 'シールド', value: v.cdn.pops === 2 ? 'あり（エッジ→シールドの2段）' : `あり（エッジ＋シールド${v.cdn.pops - 1}段の${v.cdn.pops}段）`, note: 'サーバーの手前でアクセス集中を受け止める共有キャッシュです' }
      : { label: 'シールド', value: '今回は確認できません', note: '今回は同一拠点内で完結したため、別拠点のシールドがあるかは判定できません' });
    addTerm('シールド');
  }
