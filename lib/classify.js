...
// Surrogate-Key（キャッシュタグ）。正式には空白区切りだが、ヘッダーが複数返ると toHeaderMap が
// ", " で結合するため、空白とカンマの両方で切る。同じタグが何度出ても意味は変わらないので畳む。
// 生ヘッダー(L3)は原文のまま表示するので、ここで畳むのは L2 の見せ方だけの話。
function parseSurrogateKeys(map) {
  const v = get(map, 'surrogate-key');
  if (v == null) return null;
  const list = [...new Set(String(v).split(/[\s,]+/).filter(Boolean))];
  return list.length ? list : null;
}
...
export function classify(input = {}) {
  ...
  const tags = parseSurrogateKeys(map);
