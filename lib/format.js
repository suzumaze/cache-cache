...
/** カウントダウン用。1秒ごとに再描画される場所で使うので、丸めた「約X」ではなく残りをそのまま出す
 *  （毎秒刻むのに「約」を付けると、値が動いていることと矛盾して見える）。1時間以上は秒まで見せても
 *  意味がないので formatDurationJa（約X時間）に委ねる。Age が秒精度のため±1秒の誤差は残るが、
 *  「約2分」の丸め（±30秒）より実態に近い。 */
export function formatCountdownJa(sec) {
  if (sec == null || !Number.isFinite(sec)) return null;
  const s = Math.max(0, Math.round(sec));
  if (s >= 3600) return formatDurationJa(s);
  if (s < 60) return `${s}秒`;
